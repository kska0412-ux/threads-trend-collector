...
SAMPLES = [
    ("p1", "hair_clinic_jp",
     ...
     4, 1250, ["育毛"], ["生え際 後退", "AGA"]),
    ("p2", "esthe_mika",
     "小顔になりたい人へ。\n\nマッサージより先に、噛みしめを直してください。\n\n夜、歯が触れてる時間が長い人ほどエラが張ります。",
     30, 3400, ["エステティシャン"], ["小顔", "たるみ 改善"]),
    ("p3", "ikumou_lab",
     "育毛剤って、実は「つける量」より「つける場所」。\n\n毛先じゃなく頭皮。\n当たり前だけど8割の人が間違えてます。",
     2, 85, ["ヘッドスパ"], ["育毛剤", "頭皮ケア"]),
    ("p4", "skin_pro_88",
     "毛穴の黒ずみ、ゴシゴシ洗うと悪化します。\n\n黒ずみの正体は皮脂の酸化。\n落とすんじゃなくて、酸化させない。",
     200, 8900, ["エステティシャン"], ["毛穴 ケア"]),
    ("p5", "kamiwaza_dr",
     "抜け毛が増える季節、実は秋です。\n\n夏の紫外線ダメージが3ヶ月遅れで出る。\n今抜けてるのは、7月のあなたのせい。",
     12, 620, ["育毛", "ヘッドスパ"], ["抜け毛 対策", "発毛"]),
    ("p6", "test_edge",
     'タイムスタンプ欠損のテスト投稿 <script>alert(1)</script> & "引用" も含む',
     None, 42, ["ヘッドスパ"], ["頭皮マッサージ"]),
    ("p7", "face_yoga_ne",
     "ほうれい線は「口輪筋」じゃなくて「頬の位置」。\n\n下がった頬が折れてできる線なので、口周りを鍛えても消えません。",
     1, 15, ["エステティシャン"], ["ほうれい線", "リフトアップ"]),
    ("p8", "aga_memo",
     "AGA治療、月1万円が高いと思うなら植毛の見積もり取ってみてください。\n\n桁が変わります。",
     700, 2100, ["育毛"], ["AGA"]),
    # --- 掛け合わせの絞り込み用 ---
    # 本文に判定語を含む投稿。ジャンルとANDで効くかを確かめるために
    # 同じジャンルで、判定語を含むものと含まないものを両方置く
    ("p9", "salon_keiei",
     "サロンの売上が伸びない時、まず見るのは集客じゃなくてリピートです。\n新規を追う前に、なぜ2回目が来ないかを潰す。",
     6, 900, ["エステティシャン"], ["エステティシャン 経営"]),
    ("p10", "school_note",
     "スクールを卒業しただけではサロンは回りません。\n受講中に何人に施術したかが全てです。",
     10, 400, ["セラピスト"], ["セラピスト スクール"]),
]
